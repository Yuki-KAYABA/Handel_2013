...
$$
U_{kjt}(P_{kjt}, Z\eta_{k}, 1_{kj, t-1}) = \int^{\infty}_{0} f_{kjt}(OOP)u_{k}(OOP, \hat{P_{kjt}}, Z\eta_{k}, 1_{kj, t-1})dOOP,
$$
...
$$
\hat{P_{kjt}} = TP^{y}_{jt} - S(I_{k})TP^{y}_{PPO_{1200}t}
$$
...
[^1]:確実性等価（Certainty Equivalent）は不確実性なOutcomeに対する期待効用と同等の効用をもたらす（確実な）金額。期待効用をそれに対応する金額（Monetary Value）で評価を行うことができるため、不確実性のあるマーケットの厚生評価に用いられる（Einav, Liran, Finkelstein and Cullen (2010)も参照。）。
